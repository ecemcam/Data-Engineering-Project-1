...
monthly_revenue_metrics as (
    ...
    from {{ ref('mart_revenue_daily') }} mr
    inner join {{ ref('mart_subscription_daily') }} msd
        on mr.date = msd.date
    where mr.date >= date_sub(
        (select max(date) from {{ ref('mart_subscription_daily') }}), 
        interval 30 day
    )
)
...
select
    ...
    round(
        (
            ...
            (select sum(cancellations) from {{ ref('mart_subscription_daily') }} where date >= date_sub(t1.latest_date, interval 30 day))
            / 
            -- Active subscribers 30 days ago
            nullif((select active_subscribers from {{ ref('mart_subscription_daily') }} where date = date_sub(t1.latest_date, interval 30 day)), 0)
        ) * 100
        , 2
    ...
    round(
        (
            ...
            (select avg(daily_customer_acquisition_cost) from {{ ref('mart_acquisition_efficiency') }} where date >= date_sub(t1.latest_date, interval 30 day))
            / 
